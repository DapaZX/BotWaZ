    ...
    naze.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Selamat Datang Di ${metadata.subject} @${num.split("@")[0]} ` })
                    ...
                    naze.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Telah Di Jadikan Admin ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Diberhentikan Menjadi Admin ${metadata.subject}` })
              }
            }
        } catch (err) {
            console.log(err)
